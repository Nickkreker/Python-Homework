...
class DoneCommand(BaseCommand):
    ...
    def perform(self, store):
        ...
        while True:
            try:
                selected_key = int(input('Input number: '))
            except ValueError:
                print('Bad input, try again.')
            except IndexError:
                print('Wrong index, try again.')
        ...
        selected_task = store.items[selected_key]

        print('Done task {0}: {1}'. format(selected_key, selected_task))
...
class UndoneCommand(BaseCommand):
    ...
    def perform(self, store):
        if len(store.items) == 0:
            print('There are no tasks to be undone.')
            return

        print('Select task to be undone:')
        ...
        print('Undone task {0}: {1}'. format(selected_key, selected_task))
